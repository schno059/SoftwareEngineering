...
var url = 'mongodb://ukko.d.umn.edu:12113/umdAliveDatabase';
...
module.exports.createClub = function(clubData, callback) {
  DBRef.collection('clubs').save(clubData, function(err, result){
    if(err){
	  ...
	  //console.log(doc);
	  //console.log(doc);
	  //console.log(doc.clubs);
      	  doc.clubs.push(clubData._id + "");
	  //DBRef.collection('users').update({"userID": clubData.members.admin}, { $push: { clubs: "eyyyyy" } });
...
module.exports.deleteEvent = function (eventID){
  DBRef.collection('events').findOne({"_id": mongojs.ObjectId(eventID)}, function (err, doc){
    if (err){
      ...
      DBRef.collection('clubs').find({"_id": mongojs.ObjectId(eventID)}).toArray(function (err, userDocs){
        for (i = 0; i < userDocs.length - 1; i++){
          var index = clubDoc.events(eventID);
          if (index > -1){
            clubDocs.events.splice(index, 1);
          }
          DBRef.collection('clubs').update({"_id": mongojs.ObjectId(eventID)}, clubDoc, function (err, result){
            if (err){
              console.log(err);
            } else {
              //Completed
